...
import { Award, Layers, RefreshCw, CheckCircle2, Trash2, AlertTriangle, ShieldCheck } from "lucide-react";
    ...
    <div className="space-y-8">
        ...
        <div className="lg:col-span-2 glass-panel rounded-2xl p-6 border border-slate-800/50 space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full font-mono text-[10px] text-left border-collapse">
              <thead>
                <tr className="text-slate-500 border-b border-slate-950 pb-2 uppercase tracking-wider text-[9px]">
              ...
              <tbody className="divide-y divide-slate-950/30">
                {versions.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-950/40">
                    ...
                    <td className="py-3 text-slate-400">{v.training_samples.toLocaleString()}</td>
                    <td className="py-3 text-slate-300 font-semibold">{(v.classification_accuracy * 100).toFixed(2)}%</td>
                    <td className="py-3 text-slate-300 font-semibold">{v.regression_r2.toFixed(4)}</td>
                    
                    {/* Status & Activation Action */}
                    <td className="py-3">
                      {v.is_active === 1 ? (
                        <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-accent-green border border-emerald-500/20 font-bold text-[8px] uppercase inline-flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 bg-accent-green/10 border border-accent-green/30 hover:bg-accent-green/20 text-accent-green font-bold text-[8px] uppercase rounded transition-all shadow-[0_0_8px_rgba(57,255,20,0.1)] disabled:opacity-50"
                        >
                          ACTIVATE MODEL
                        </button>
                      )}
                    ...
                    <td className="py-3 text-right">
                      {v.version === "v1.0" || v.version === "v1.1" ? (
                        <span className="px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-slate-500 font-bold text-[8px] uppercase">
                          PERMANENT BASELINE
                        </span>
                      ) : v.is_active === 1 ? (
                        ...
                        <button
                          ...
                          disabled={actionLoading}
                          className="px-2.5 py-1 bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 text-red-400 font-bold text-[8px] uppercase rounded transition-all inline-flex items-center gap-1 disabled:opacity-50"
                        >
      ...
      {deleteConfirmStep !== null && deleteTargetVersion && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-panel rounded-2xl p-6 border border-slate-800 max-w-md w-full font-mono space-y-6 shadow-2xl">
            
            {/* Step 1 Confirmation */}
            {deleteConfirmStep === 1 && (
              <>
                <div className="flex items-center gap-3 text-amber-400 border-b border-slate-900 pb-3">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  Are you sure you want to delete retrained model version <strong className="text-white">{deleteTargetVersion}</strong>? 
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold hover:bg-amber-500/30 rounded-lg text-xs"
                  >
            ...
            {deleteConfirmStep === 2 && (
              <>
                <div className="flex items-center gap-3 text-red-400 border-b border-slate-900 pb-3">
                ...
                <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400">
                    This will permanently delete the compiled binary file (<code className="text-slate-300">tomato_freshness_pipeline_{deleteTargetVersion}.pkl</code>) and associated training snapshots. This action CANNOT be undone.
                  </p>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-bold rounded-lg text-xs shadow-[0_0_15px_rgba(239,68,68,0.3)] disabled:opacity-50"
                  >
