    ...
    <div className="space-y-8">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 pb-4 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white font-mono tracking-wide uppercase flex items-center gap-2">
            <CheckSquare size={24} className="text-emerald-600 dark:text-accent-green" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-slate-800/40 font-mono shadow-sm">
          <span className="text-[9px] uppercase tracking-widest text-slate-500 block mb-1 font-semibold">TOTAL AUDITED SAMPLES</span>
          <span className="text-3xl font-extrabold text-slate-900 dark:text-white">{stats.total_audited_samples}</span>
          <span className="text-[9px] text-slate-500 dark:text-slate-400 block mt-1">Ready for retraining</span>
        </div>
        <div className="glass-panel rounded-2xl p-5 border border-slate-200 dark:border-slate-800/40 font-mono shadow-sm">
              ...
              {verifiedRecords.length > 0 ? (
                verifiedRecords.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/20">
                    ...
                    <td className="p-2.5 font-bold text-slate-900 dark:text-white">{row.tomato_id || "N/A"}</td>
                    <td className="p-2.5">{row.position || "N/A"}</td>
                    <td className="p-2.5 text-slate-600 dark:text-slate-400">{row.blue?.toFixed(1)} / {row.nir?.toFixed(1)}</td>
                    <td className="p-2.5 text-slate-600 dark:text-slate-400">{row.ndvi?.toFixed(4)}</td>
                    <td className="p-2.5 text-slate-500">{row.predicted_category}</td>
                    <td className="p-2.5">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase ${
                        row.actual_category === "Fresh" ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20" :
                        row.actual_category === "Aging" ? "bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20" :
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-[10px] font-mono text-slate-600 dark:text-slate-400 mt-2 bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-slate-200 dark:border-slate-900">
              ...
              <span>
                ...
                <strong className={stats.retraining_readiness ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-amber-600 dark:text-amber-400 font-bold"}>
